...
api_key='' # 請修改此處
secret_key='' # 請修改此處
id_strategy = "109034011_daylong" # 自行修改
...
def strategy():
    ...
    #     elif (trader.tick[-1]['close'] < self_sl):
    #         trader.sell(size=1)
    ...
    ema250 = ta.ema(close=df_TXF['Close'], length=250)
    ...
    # if self_position == 'None' or self_position == 'Sell':

    #     if (df_TXF['Close'][-2] < var2) and (df_TXF['Close'][-1] > var2) and self_var3[-1] > 0 and condition2:
    ...
    #     if self_var3[-1] < 0:
    #         trader.sell(size=1)
    #     elif datetime.datetime.now().time() > datetime.time(13, 25):
    #         trader.sell(size=1)
    ...
    if self_position == 'None':
        ...
        #     trader.sell(size=1)
        #     maxProfit = 0
        if datetime.datetime.now().time() >= datetime.time(9, 00) and datetime.datetime.now().time() <= datetime.time(13, 25):
            if (df_TXF['Close'][-1] > ema250[-1]) and (record['大戶買進'][-1] > 600) and (record['大戶買進'][-1] - record['大戶買進'][-2] > 150):
                trader.buy(size=1)
    ...
    if self_position == 'Buy':
        ...
        if record['大戶買進'][-1] - record['大戶買進'][-2] <= -580:
            trader.sell(size=1)
            maxProfit = 0
        elif datetime.datetime.now().time() > datetime.time(13, 25): # 當沖
            trader.sell(size=1)
            maxProfit = 0
