...
api_key='' # 請修改此處
secret_key='' # 請修改此處
id_strategy = "109034011_dayshort" # 自行修改
...
def strategy():
    ...
    if self_position == 'None':
        ...
        if datetime.datetime.now().time() >= datetime.time(9, 00) and datetime.datetime.now().time() <= datetime.time(13, 25):
            if (df_TXF['Close'][-1] < ema280[-1]) and (record['大戶買進'][-1] < -520) and (record['大戶買進'][-1] - record['大戶買進'][-2] <= -270):
                trader.sell(size=1)
    ...
    if self_position == 'Sell':
        ...
        if record['大戶買進'][-1] - record['大戶買進'][-2] >= 460:
            trader.buy(size=1)
            maxProfit = 0
