...
api_key='' # 請修改此處
secret_key='' # 請修改此處
id_strategy = "109034011_nightlong" # 自行修改
...
def strategy():
    ...
    if self_position == 'None':
        if datetime.datetime.now().time() > datetime.time(21, 25) or datetime.datetime.now().time() <= datetime.time(4, 00):  
            if (record['大戶買進'][-1] > 650) and (record['大戶買進'][-1] - record['大戶買進'][-2] > 70):
                trader.buy(size=1)
