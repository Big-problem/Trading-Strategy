...
class pytrader:
    ...
    def __init__(self, strategy, api_key, secret_key, ca_path=None, ca_passwd=None, person_id=None, simulation=True, token=True, contract="TXF"):
        ...
        self.api = api
        self._price_type = 'MKT'
        self._order_type = 'ROD'
        self._octype = 'New'
        ...
        self.trades['exit_time'] = pd.to_datetime(
            self.trades['exit_time'], format='%Y-%m-%d %H:%M:%S')
        self.trades['exit_time'] = self.trades['exit_time'].dt.to_pydatetime()
    ...
    def contract(self, code):
        ...
        self.tick = msg_queue['tick']
        self.bidask = msg_queue['bidask']

        while (len(self.tick) == 0):
            time.sleep(0.1)
        ...
        if (self.simulation == False):
            ...
            if (self.token == True):
                order = api.Order(
                    ...
                    order_type=getattr(
                        sj.constant.OrderType, self._order_type),
                ...
                order = api.Order(
                    ...
                    price_type=getattr(
                        sj.constant.FuturesPriceType, self._price_type),
                    order_type=getattr(
                        sj.constant.FuturesOrderType, self._order_type),
                    octype=getattr(sj.constant.FuturesOCType, self._octype),
                    account=api.futopt_account
                )
    ...
    def position(self):
        ...
        if (sum(item['size'] for item in self.orders) != 0):
            ...
            if position['size'] > 0:
                ...
                position['pl_pct'] = (
                    (self.trades[-1]['entry_price']*0.99998 - float(msg_queue['bidask'][-1]['ask_price'][0])*1.00002) / (self.trades[-1]['entry_price']))
